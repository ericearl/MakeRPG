...
class SkillForm(forms.Form):
    def __init__(self,*args,**kwargs):
        self.skillname = kwargs.pop('skill')
        super(SkillForm,self).__init__(*args,**kwargs)

        span = sorted(CharacterSkill.objects.filter(skill__name=self.skillname).values_list('current', flat=True).distinct())

        self.fields['minimum'] = forms.TypedChoiceField(
...
class StatisticForm(forms.Form):
    def __init__(self,*args,**kwargs):
        self.statname = kwargs.pop('statistic')
        super(StatisticForm,self).__init__(*args,**kwargs)

        span = sorted(CharacterStatistic.objects.filter(statistic__name=self.statname).values_list('current', flat=True).distinct())
